...
class DefaultController extends Controller
{
    ...
    public function dirtree($dir, $regex = '', $ignoreEmpty = false, $depth = 0)
    {
        ...
        foreach ($origin as $item) {
            ...
            if ($item->isDir() && $searchFolders) {
                ...
                foreach ($obj as $info) {
                    $path = $info->isDir() ? [
                        $info->getFilename() => [
                            ...
                            'config' => collect($this->getConfig($info->getRealPath()))->filter(function (
                                    $value,
                                    $key
                                ) {
                                    return $value != null;
                    ...
                    for ($depth = $obj->getDepth() - 1; $depth >= 0; $depth--) {
                        ...
                        switch ($info->getExtension()) {
                            case 'html':
                            case 'twig':
                                $file['templateRender'] = $templates[] = $file;
                                break;
                            case 'js':
                            case 'css':
                            case 'vue':
                ...
                $arr = collect($arr)->filter(function ($item) {
    ...
    public function actionPageRender(string $page)
    {
        $pages = CraftGonzo::getInstance()->getSettings()->pages;

        $options = isset($pages[$page]['options']) ? $pages[$page]['options'] : [];

        return $this->renderTemplate('patternlib/pages/'.$page, $options);
    }
